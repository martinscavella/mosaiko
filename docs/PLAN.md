...
| T3.6 | ✅ **FATTO 2026-07-18** (`src/lib/transactionTypes.ts` con 25 tipi canonici/kind/segno + normalizzazione legacy; consumato da modale, combobox, import, filtri, display; migration 20260718_transaction_type_taxonomy: normalizzati 603 valori legacy inglesi + CHECK + NOT NULL, saldi invariati verificati via hash; test sul segno in transactionTypes.test.ts) — Tassonomia `transaction_type` unica | P1 | — | M | Zero liste stringa duplicate; test sul segno |
| T3.7 | ✅ **FATTO 2026-07-18** (migration 20260718_fix_signup_profile_metadata: handle_new_user/handle_user_update leggono chiavi snake_case + birth_date/phone_number; drop trigger morto create_profile_on_user_registration; rimosso INSERT client-side; trigger verificato con signup simulato in transazione) — Fix signup (critico per D2): rimuovere `createUserProfile` client-side; estendere `handle_new_user()` ai metadata mancanti (birth_date, phone) o UPDATE post-signup; rimuovere log residui | P1 | T1.4 | S | Registrazione reale crea profilo completo senza errori (resta il test manuale T1.4) |
| T3.8 | ✅ **FATTO 2026-07-18** (cache espone `account_id` su refunds/fundsTransfer; accounts page, usage map, dettaglio account e breakdown per-conto in Reports matchano per id; verificato 0 righe con account_id NULL sul DB live. NB: il multiselect filtro conti in Reports resta keyed per nome → da sistemare con T4.1/T4.2) — Join per `account_id` invece di `account_name` | P1 | — | S | Statistiche corrette con due conti omonimi |
...
| T4.8 | ✅ **FATTO 2026-07-18** (nuovi `lib/supabase/client.ts` (createBrowserClient) e `lib/supabase/server.ts` (createServerClient); migrati tutti i call site client e il route handler; rimossi i 4 pacchetti deprecati auth-helpers-*/auth-ui-*; middleware di sessione non necessario: app tutta client-side, l'unica API route valida il JWT con getUser(). NB: al deploy le sessioni esistenti scadono (formato cookie diverso) → re-login una tantum. Verifica e2e login/logout/RLS rimandata a fine sviluppo con T1.4) — Migrazione auth a `@supabase/ssr` | P1 | T2.1, T3.10 | L | Login/logout/refresh + RLS verificati e2e |
